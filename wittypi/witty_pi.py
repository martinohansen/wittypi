...
class WittyPi:
    ...
    def __repr__(self) -> str:
        return "Witty Pi (State: {}, {}, {}, Available Schedules: {})".format(
            self.path, self.next_start, self.next_shutdown, self.available_schedules
        )
    ...
    def _check_hardware(self):
        for file in self.required_files:
            if not os.path.isfile(os.path.join(self.path, file)):
                raise Exception(
                    "Witty Pi script '{}' does not exist in '{}'.".format(
                        file, self.path
                    )
                )
    ...
    def _get_startup(self):
        ...
        local_t = self.__run_util_function(
            "get_local_date_time", "'{}'".format(t[1].replace("\n", ""))
        )
        if len(local_t[1]) < 5:
    ...
    def _get_shutdown(self):
        ...
        if len(local_t[1]) < 5:
            ...
            self.next_shutdown = WittyTime(
                *self._split_date_str(local_t[1]), name="shutdown"
            )
    ...
    def activate_schedule(self, index):
        ...
        if index - 1 > len(self.available_schedules):
            raise Exception(
                "Index Error: No schedule at index {} of self.available_schedules.".format(
                    index
                )
            )
        copyfile(
            self.path + "schedules/" + self.available_schedules[index],
            self.path + "/schedule.wpi",
        )
        sp.run(
            ["sudo", "bash", "-c", "source " + self.path + "runScript.sh"],
            stdout=sp.PIPE,
        )
        self._get_startup()
        self._get_shutdown()
    ...
    def reset(self, option):
        ...
        if option not in options:
            raise Exception(
                'Error: Chose one of these options: "startup", "shutdown", "schedule", "all"'
            )
        if option != "shutdown":
    ...
    def _compare_times(self):
        ...
        self.timedelta = (
            sys_time - datetime.datetime.strptime(rtc, "%a %d %b %Y %H:%M:%S %Z")
        ).total_seconds()
...
class WittyTime:
    ...
    def __check_format(self, args):
        ...
        if args[0] == 0:
            ...
            raise Exception(
                "Format Error: Day must be an integer between 1-31 or '??' for wildcard."
            )
        else:
            self.day = args[0]
        if len(args) == 1:
            ...
            if not self.day == "??":
                raise Exception(
                    "Format Error: Wildcards must be used from left to right."
                )
            else:
                self.hour = args[1]
        else:
            raise Exception(
                "Format Error: Hour must be an integer between 0-24 or '??' for wildcard."
            )
        if len(args) == 2:
            self.minute = 00
            self.second = 00
            ...
            raise Exception(
                "Format Error: Wildcards should only be used for Days and Hours. Not for Minutes"
            )
        else:
        ...
        if len(args) == 3:
            ...
            raise Exception(
                "Format Error: Wildcards should only be used for Days and Hours. Not for Seconds"
            )
        else:
    ...
    def __to_datetime(self):
        ...
        if self.day == "??":
            if (
                next["hour"] < now.hour
                or next["hour"] == now.hour
                and next["minute"] < now.minute
            ):
                next["day"] = now.day + 1
    ...
    def __repr__(self):
        return "WittyTime(type={}, day={}, hour={}, minute={}, second={}, as_date={})".format(
            self.name, self.day, self.hour, self.minute, self.second, self.as_date
        )
